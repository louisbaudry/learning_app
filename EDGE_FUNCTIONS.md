...
**Purpose:** Validate a one-time device link code and link the caller's own
anonymous auth session to the child's student profile.

**Rewritten 2026-09-15:** the original version created a *separate* auth
user server-side via `auth.admin.createUser()` and never actually returned
usable session credentials for it — a dead end that blocked every device
link attempt (confirmed while building the first end-to-end test). Fixed
to use standard Supabase anonymous auth instead — see request flow below.

**Request:**
```typescript
POST /functions/v1/redeem-link-code
Authorization: Bearer <anonymous session JWT>  // from supabase.auth.signInAnonymously(), called client-side FIRST
...
```typescript
{
  ...
  // No token is returned — the caller already holds its own session from
  // signInAnonymously(); reuse that for all future requests.
}
```
...
- `auth.uid()` comes from the caller's own verified JWT, never from
  anything client-supplied — the function only uses the `service_role` key
  for the `student_devices` insert / `device_link_codes` update, since
  `student_devices` intentionally has no RLS policy letting a fresh
  anonymous user insert its own row
- Idempotent: re-calling with an already-linked session's JWT (e.g. a page
  reload right after linking) returns the existing link instead of erroring
...
3. Child's client calls `supabase.auth.signInAnonymously()` itself, getting
   a real session (access/refresh tokens) it persists
4. Client calls `redeem-link-code` WITH that session's JWT
5. Function validates the code and links `auth.uid()` (the caller's own
   anonymous user) to the student via `student_devices`
6. Client reuses its own session for all future requests
